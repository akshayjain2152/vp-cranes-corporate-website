...
function Logo({ light = false }: { light?: boolean }) { return <a href="#home" className="brand" aria-label="VP CRANES home"><Image src="/images/logo.png" alt="VP CRANES" width={46} height={46} priority /><span className={light ? "brand-light" : ""}>VP <b>CRANES</b><small>HEAVY LIFTING SOLUTIONS</small></span></a>; }
...
function WhyUs() { return <section id="why-us" className="section why"><div className="container"><SectionHeading eyebrow="Why VP CRANES" title="Clear thinking. Steady hands. Confident outcomes.">We bring the practical disciplines of a trusted field partner to every site, from the first review to safe completion.</SectionHeading><div className="strength-grid">{strengths.map(({ title, text, Icon }, i) => <Reveal key={title} delay={i * .06}><article><Icon size={24} strokeWidth={1.4}/><span>0{i + 1}</span><h3>{title}</h3><p>{text}</p></article></Reveal>)}</div></div></section> }

function Clientele() { return <section id="clientele" className="section clientele"><div className="container"><SectionHeading eyebrow="Trusted by teams on the ground" title="Relationships built lift by lift."/><div className="client-layout"><Reveal><blockquote><Quote size={36}/><p>“The best partnerships are made in the work: clear scope, capable people and a result everyone can stand behind.”</p><footer>VP CRANES <span>·</span> Our working philosophy</footer></blockquote></Reveal><div className="client-grid">{clients.map((client, i) => <Reveal key={client} delay={i * .025}><div><span>{client.split(" ").filter(Boolean).slice(0, 2).map((word) => word[0]).join("")}</span><p>{client}</p></div></Reveal>)}</div></div></div></section> }

function Gallery() { return <section className="gallery" aria-label="VP CRANES project gallery"><div className="gallery-intro"><p className="eyebrow eyebrow-light">In the field</p><h2>Strength in<br/><em>every frame.</em></h2><a href="#contact" className="text-link light">Plan your next lift <ArrowRight size={17}/></a></div><div className="gallery-grid">{gallery.map((image, i) => <div className={`gallery-image gallery-${i + 1}`} key={image}><Image src={`https://www.vpcranes.com/images/${image}`} alt="VP CRANES field project" fill sizes="(max-width: 700px) 50vw, 25vw"/></div>)}</div></section> }
